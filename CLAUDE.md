...
→ WebSocket으로 태블릿에 전송 → 태블릿 브라우저 Canvas에서 1인칭 후방 시점 렌더링
```
...
- **진짜 타는 느낌**: 이 프로젝트의 최우선 목표. 아래 세 가지가 핵심.
  ...
  - **시점**: 자전거 뒤·위에서 앞으로 뻗은 도로를 보는 의사 3D(pseudo-3D) 시점. 언덕이 눈에 보인다.
...
자전거 뒤·약간 위에서 앞으로 뻗은 도로를 보는 시점. 고전 레이싱 게임(OutRun) 방식의 **세그먼트 투영**:
...
- 화면 하단 중앙, 뒤에서 본 모습 (Canvas path로 그림).
