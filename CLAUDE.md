...
- **배경 그림은 "뒤에서 본 시점"으로 받아야 한다.** 카메라가 라이더 뒤 아니면 라이더의 눈이고
  캐릭터 스프라이트도 뒷모습이라, 길은 화면 아래에서 소실점으로 뻗어야 한다. 옆에서 본
  횡스크롤 입면도를 받으면 길과 90도로 어긋난 채 달리는 그림이 된다 (한강 맵 `hanriver`가
  실제로 그랬다 — 아트는 훌륭한데 시점이 다르다). 자세한 조건은 `static/maps/README.md`.
- **받은 에셋은 말을 믿지 말고 재본다.** 한강 맵은 "seamless"라고 왔지만 좌우 끝 색차가
  near 62/255였고, mid·near는 알파가 가로 직선으로 잘려 하늘에 선이 그어졌고,
  크로마키 잔재로 반투명 가장자리의 100%(mid)·44%(near)가 초록이었다.
  → 타일 이음매·알파 계단·초록 잔재 세 가지는 넣기 전에 확인한다 (`tools/despill.py`).
