...
- 1인칭 콕핏은 카메라 자식(핸들바·후드·스템·속도계) + **후드를 잡은 장갑**. 몸통·다리·팔뚝은 넣지 않음.
  팔뚝을 원기둥으로 이어봤더니 카메라에 가까운 쪽이 굵어져 기둥처럼 보이고 드롭을 가렸다 → 손만 남김.
- 테마 팔레트는 Gemini 아트 디렉션 기반 (산=노을, 해변=아침, 아케이드=야간 신스웨이브).
- 저사양 태블릿(Lenovo TB-X606F, PowerVR GE8320) 대비: 안티앨리어싱 끔, 안개로 거리 제한, 길가 오브젝트는 인스턴싱.
