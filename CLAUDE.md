...
메뉴 오른쪽 위 "그래픽" 칩 또는 `G`키로 3D ↔ 2D 전환 (localStorage `pq.gfx`, **기본 3D**).
...
- **1인칭 콕핏은 3D 메시가 아니라 `static/cockpit.js`(2D 오버레이)가 그린다.**
  팔·손·핸들바는 그려둔 PNG 스프라이트(`static/cockpit-arms.png`), 속도계는 코드
  (숫자가 매 프레임 바뀌므로). 무릎은 배경 없는 다리 PNG가 생기면 켠다 (`SHOW_KNEES`).
- **AI가 만든 스프라이트는 알파 채널이 없을 수 있다.** 투명 배경을 요청하면 체크무늬를
  *픽셀로 그려서* 준다 (실제로 그랬다: colortype=2, RGB). 그대로 얹으면 회색 격자가 보인다.
  → `tools\sprite_alpha.py`로 진짜 알파로 바꾼 뒤 `static/`에 넣는다. Pillow가 필요하지만
  게임 실행에는 안 쓰이므로 requirements.txt에는 넣지 않는다.
- 받은 무릎 PNG에는 도로 배경과 핸들바가 같이 찍혀 있었다 → 3D 위에 얹으면 도로가 두 겹이
  되고 핸들바가 둘이 된다. **스프라이트는 "그 부위만" 잘려 있어야 한다.**
